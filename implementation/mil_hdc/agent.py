...
class MILHDCAgent(Agent):
    def __init__(self, node_dim: int, action_dim: int, config: Config):
        ...
        if config.graph:
            self._embed = RelEncoder(dim=config.hypervec_dim, node_dim=node_dim)
            self._memory = GraphMemoryBuffer(buffer_length=config.buffer_size,
                                             sample_size=config.sample_size,
                                             mask_subnet_state_index=True)  # TODO: Mariam -- look into
        else:
        ...
        self._q_target = QFunctionTarget(qfunction=self._q_func, tau=config.tau)

        self._policy = Actor(dim=config.hypervec_dim, action_dim=action_dim)
        ...
        self._alpha = sac.Alpha(start=config.target_entropy_start,
                                ...
                                max_steps=config.max_steps,  # Hard cutoff
    ...
    def param_update(self) -> dict[str: float]:
        ...
        with torch.no_grad():
            ...
            _, next_prob, next_log_prob = self._policy.sample_action(next_state_embed, next_batch_index,
                                                                     trans.next_state_index)

            next_log_prob = next_log_prob / torch.log(number_devices * self._action_dim).view(-1,
                                                                                              1)  # Normilize by the maximum possible entropy
        ...
        q1_dif, q2_dif = sac.q_func_loss(cur_q1,
                                         ...
                                         next_log_prob,
                                         trans.reward,
                                         self._alpha(),  # TODO: Mariam -- need to check whether I need to call item()
        ...
        # Find what devices correspond to the chosen action and then select only the device embedding for that action
